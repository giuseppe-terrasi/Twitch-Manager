...
namespace TwitchManager.Services.Abstractions
{
    ...
    {
        Task<ICollection<StreamerModel>> GetAllAsync(CancellationToken cancellationToken = default);

        Task<ICollection<StreamerModel>> GetAllByUserAsync(CancellationToken cancellationToken = default);
